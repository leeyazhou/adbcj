...
public class FieldDecodingState<T> extends DecoderState {
  ...
  public FieldDecodingState(MySqlConnection connection, Row.RowDecodingType decodingType, int expectedAmountOfFields,
  ...
  public ResultAndState parse(int length, int packetNumber, BoundedInputStream in, Channel channel) throws IOException {
    ...
    if (expectedAmountOfFields > (fieldNo + 1)) {
      return result(new FieldDecodingState<T>(connection, decodingType, expectedAmountOfFields, newFields, eventHandler,
          accumulator, callback, entry, failure), resultSetFieldResponse);
    } else {
      return result(
          new FieldEof<T>(connection, decodingType, newFields, eventHandler, accumulator, callback, entry, failure),
