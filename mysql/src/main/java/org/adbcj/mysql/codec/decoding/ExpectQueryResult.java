...
public class ExpectQueryResult<T> extends ResponseStart {
  ...
  public ExpectQueryResult(MySqlConnection connection, Row.RowDecodingType decodingType, ResultHandler<T> eventHandler,
  ...
  @Override
  protected ResultAndState parseAsResult(int length, int packetNumber, BoundedInputStream in, int fieldCount)
      throws IOException {
    ...
    return result(
        new FieldDecodingState<T>(connection, decodingType, expectedFieldPackets, new ArrayList<>(), eventHandler,
