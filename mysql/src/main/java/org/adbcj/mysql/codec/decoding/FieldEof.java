...
class FieldEof<T> extends DecoderState {
  ...
  public FieldEof(MySqlConnection connection, Row.RowDecodingType decodingType, List<MysqlField> fields,
  ...
  public ResultAndState parse(int length, int packetNumber, BoundedInputStream in, Channel channel) throws IOException {
    ...
    return result(new Row<T>(connection, decodingType, fields, eventHandler, accumulator, callback, entry, failure),
        fieldEof);
  }
