package com.ly.train.flower.db.tck;

import java.util.Random;
import com.ly.train.flower.db.api.Connection;
import com.ly.train.flower.db.api.ConnectionManager;
import com.ly.train.flower.db.api.ConnectionManagerProvider;

/**
 *
 */
public class PopulateLarge {

  public static void main(String[] args) throws Exception {
    ConnectionManager mysqlCM = ConnectionManagerProvider
        .createConnectionManager("asyncdb:mysqlnetty://localhost/asyncdbtck", "asyncdbtck", "asyncdbtck");
    ConnectionManager pgCM = ConnectionManagerProvider
        .createConnectionManager("asyncdb:postgresql-netty://localhost/asyncdbtck", "asyncdbtck", "asyncdbtck");

    Connection mysql = mysqlCM.connect().get();
    Connection pg = pgCM.connect().get();

    final String insertTemplate = "INSERT INTO large (a, b, c) VALUES ('%s', '%s', '%s')";
    for (int i = 0; i < 998; i++) {
      String a = randString();
      String b = randString();
      String c = randString();
      final String insert = String.format(insertTemplate, a, b, c);
      mysql.executeUpdate(insert).get();
      pg.executeUpdate(insert).get();
    }
    // mysql.close(false).get();
    // pg.close(false).get();
    // mysqlCM.close(true);
    // pgCM.close(true);
  }

  private static String randString() {
    Random rand = new Random();
    StringBuilder sb = new StringBuilder();
    for (int i = 0; i < 255; i++) {
      char c = (char) (rand.nextInt(26) + 65);
      sb.append(c);
    }
    return sb.toString();
  }

}
