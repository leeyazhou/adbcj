...
public class SelectTest extends AbstractWithConnectionManagerTest {
  ...
  public void testWorksWithCallback() throws Exception {
    ...
    Future<StringBuilder> resultFuture =
        connection.executeQuery("SELECT str_val FROM simple_values " + "WHERE str_val LIKE 'Zero'",
            buildStringInCallback(), new StringBuilder());
  ...
  public void testExceptionInCallbackHandler() throws Exception {
    ...
    while (stillHasToExplore) {
      ...
      Future<StringBuilder> resultFuture = connection.executeQuery(
          "SELECT str_val FROM simple_values " + "WHERE str_val LIKE 'Zero'", faultyHandler, new StringBuilder());
      try {
        resultFuture.get();
  ...
  public void testTwoSelectsAfterEachOther() throws Exception {
    ...
    Future<ResultSet> firstCall =
        connection.executeQuery("SELECT str_val FROM simple_values " + "WHERE str_val LIKE 'One'");
    Future<ResultSet> nextCall =
        connection.executeQuery("SELECT str_val FROM simple_values " + "WHERE str_val LIKE 'Two'");
  ...
  public void testExceptionInCallbackHandlerDoesNotAffectNextCall() throws Exception {
    ...
    Future<StringBuilder> errorCause =
        connection.executeQuery("SELECT str_val FROM simple_values " + "WHERE str_val LIKE 'Zero'",
            new ExceptionOnCall(0), new StringBuilder());
    ResultSet nextCall =
        connection.executeQuery("SELECT str_val FROM simple_values " + "WHERE str_val LIKE 'Two'").get();
    try {
      errorCause.get();
