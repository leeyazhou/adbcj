 ...
 */
package com.ly.train.flower.db.h2.h2;
...
public class SHA256 {
  ...
  /**
   * Calculate the hash code by using the given salt. The salt is appended after
   * the data before the hash code is calculated. After generating the hash code,
   * the data and all internal buffers are filled with zeros to avoid keeping
   * insecure data in memory longer than required (and possibly swapped to disk).
   *
   ...
   * character. Both the user name and the password are encoded to a byte array
   * using UTF-16. After generating the hash code, the password array and all
   * internal buffers are filled with zeros to avoid keeping the plain text
   ...
   *
   * @param data the data to hash
   * @param nullData if the data should be filled with zeros after calculating the
   *        hash code
   * @return the hash code
   */
  public static byte[] getHash(byte[] data, boolean nullData) {
    ...
    int[] w = new int[64];
    int[] hh = {0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19};
