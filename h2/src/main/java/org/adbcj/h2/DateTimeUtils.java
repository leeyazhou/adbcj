...
public class DateTimeUtils {
  ...
  public static Timestamp convertDateValueToTimestamp(long dateValue, long nanos) {
    ...
    long h = m / 60;
    m -= h * 60;
    long ms = DateTimeUtils.getMillis(TimeZone.getDefault(), DateTimeUtils.yearFromDateValue(dateValue),
        DateTimeUtils.monthFromDateValue(dateValue), DateTimeUtils.dayFromDateValue(dateValue), (int) h, (int) m,
        (int) s, 0);
  ...
  /**
   * Calculate the milliseconds since 1970-01-01 (UTC) for the given date and time
  ...
  private static long getTimeTry(Calendar c, boolean lenient, int year, int month, int day, int hour, int minute,
      int second, int millis) {
    c.clear();
