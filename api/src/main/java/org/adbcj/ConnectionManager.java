...
/**
 * Manages a set of {@link Connection} instances. A database connection is
 * established by invoking {@link #connect()}. All the connections managed by
 * this connection manager can be closed using {@link #close(CloseMode)}.
 * <p>
 * The connection manager holds heavy wait resources like connections, thread
...
public interface ConnectionManager extends AsyncCloseable {
   ...
   *
   * @return a future object that will complete when the database connection has
  ...
  /**
   * Closes all the database connections managed by this {@code ConnectionManager}
   * and releases any resources used for managing asynchronous database
   * connections.
   * <p>
   * This will close the connections gracefully
   * {@link CloseMode#CLOSE_GRACEFULLY}. Running operations will finish before the
   * manager is closed. The behavior is the same as calling
   * close(CloseMode.LOSE_GRACEFULLY)
   *
   ...
   *
   * @return a future object that will complete when all database connections
   *         managed by this {@code ConnectionManager} have closed.
   */
