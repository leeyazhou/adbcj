 ...
 *     ConnectionManagerProvider.createConnectionManager("asyncdb:db-specific/url", "user", "password");
...
public interface Connection extends AsyncCloseable {
   ...
   * <p>
   * The transaction is started immediately and all subsequent operations are
   * executed in a transaction, until {@link #commit()} or {@link #rollback()} is
   * called. This call also doesn't block: The communication to establish the
   * transaction are executed in the background before the next call on the
   * connection object.
   */
  ...
  /**
   * Same as {@link #beginTransaction()}. The callback is called when confirmation
   * arrived for the transaction begin. However, right after calling this method,
   * other commands can be sends and will be part of the transactions
   */
  ...
  /**
   * Commits this transaction. Callback will complete if the transaction has been
   * committed successfully, or contain the error when the commit failed.
   * <p>
  ...
  /**
   * Commits this transaction. The callback completes when the transaction has
   * been rolled back successfully. Rollbacks should not fail, however it it does
   * the future will contain the error.
   * <p>
   * Rolling back a transaction might cancel all requests which have been made for
   * this transaction.
   * <p>
  ...
  default <T> CompletableFuture<T> executeQuery(String sql, ResultHandler<T> eventHandler, T accumulator) {
   ...
   * <p>
   * This will close the connection gracefully {@link CloseMode#CLOSE_GRACEFULLY}.
   * Running operations will finish before the manager is closed. However no new
